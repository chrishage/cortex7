...
SELECT
  ...
  mcha.batch_id AS batch_id,
  mcha.xpcbt AS single_unit_batch_indicator_xpcbt,
  ...
  mcha.creation_datetime AS created_on_timestamp_creation_datetime,
  mcha.lastchange_datetime AS last_change_timestamp_lastchange_datetime,
  CAST(NULL AS STRING) AS serialization_type_sttpec_sertype,
  CAST(NULL AS STRING) AS batch_production_plant_sttpec_plant,
  CAST(NULL AS STRING) AS synchronization_time_stamp_sttpec_synctime,
  CAST(NULL AS STRING) AS synchronization_active_sttpec_syncact,
  GREATEST(
