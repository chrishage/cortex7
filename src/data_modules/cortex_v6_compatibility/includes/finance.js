...
/**
 * Helper to generate the SQL expression for Net Due Date Calculation.
 * Implements the legacy NetDueDateCalc UDF.
 */
function netDueDate(koart, zfbdt, bldat, shkzg, rebzg, zbd3t, zbd2t, zbd1t) {
  return `
    DATE_ADD(
      IF(${koart} = 'D' AND ${zfbdt} IS NULL, ${bldat}, ${zfbdt}),
      INTERVAL CAST(
        IF(${koart} = 'D' AND ${shkzg} = 'H' AND ${rebzg} IS NULL, 0,
          CASE
            WHEN ${koart} = 'D' AND ${zbd3t} IS NOT NULL THEN ${zbd3t}
            WHEN ${koart} = 'D' AND ${zbd2t} IS NOT NULL THEN ${zbd2t}
            WHEN ${koart} = 'D' AND ${zbd1t} IS NOT NULL THEN ${zbd1t}
            WHEN ${zbd1t} IS NULL THEN 0
          END
        ) AS INT64
      ) DAY
    )
...
/**
 * Helper to generate the SQL expression for Cash Discount 1 Due Date.
 * Implements the legacy DueDateForCashDiscount1 UDF.
 */
function dueDateForCashDiscount1(koart, zfbdt, bldat, shkzg, rebzg, zbd3t, zbd2t, zbd1t) {
  return `
    CASE
      WHEN ${zbd1t} IS NOT NULL OR ${zbd2t} IS NOT NULL THEN DATE_ADD(${zfbdt}, INTERVAL CAST(${zbd1t} AS INT64) DAY)
      ELSE ${netDueDate(koart, zfbdt, bldat, shkzg, rebzg, zbd3t, zbd2t, zbd1t)}
    END
  `;
}

/**
 * Helper to generate the SQL expression for Cash Discount 2 Due Date.
 * Implements the legacy DueDateForCashDiscount2 UDF.
 */
function dueDateForCashDiscount2(koart, zfbdt, bldat, shkzg, rebzg, zbd3t, zbd2t, zbd1t) {
  return `
    CASE
      WHEN ${zbd2t} IS NOT NULL THEN DATE_ADD(${zfbdt}, INTERVAL CAST(COALESCE(${zbd2t}, 0) AS INT64) DAY)
      ELSE ${netDueDate(koart, zfbdt, bldat, shkzg, rebzg, zbd3t, zbd2t, zbd1t)}
    END
  `;
}
...
/**
 * Helper to generate the SQL expression for Leap Year check.
 * Bypasses the need for external UDFs by inlining the mathematical logic.
 */
function isLeapYear(yearExpression) {
  return `(MOD(CAST(${yearExpression} AS INT64), 4) = 0 AND (MOD(CAST(${yearExpression} AS INT64), 100) != 0 OR MOD(CAST(${yearExpression} AS INT64), 400) = 0))`;
}
...
function getAccountingDocuments(ctx, sapDatasetId, targetDatasetId) {
  return `(
(WITH currency_decimal AS (
  ${currencyHelper.getCurrencyDecimalCTE(ctx, sapDatasetId)}
),
BSEG AS (
    -------------------------------------------------------------------------
...
SELECT
  ...
  ${netDueDate('BSEG.KOART', 'BSEG.ZFBDT', 'BKPF.BLDAT', 'BSEG.SHKZG', 'BSEG.REBZG', 'BSEG.ZBD3T', 'BSEG.ZBD2T', 'BSEG.ZBD1T')} AS NetDueDateCalc, ${netDueDate('BSEG.KOART', 'BSEG.ZFBDT', 'BKPF.BLDAT', 'BSEG.SHKZG', 'BSEG.REBZG', 'BSEG.ZBD3T', 'BSEG.ZBD2T', 'BSEG.ZBD1T')} AS NetDueDate,
  ${dueDateForCashDiscount1('BSEG.KOART', 'BSEG.ZFBDT', 'BKPF.BLDAT', 'BSEG.SHKZG', 'BSEG.REBZG', 'BSEG.ZBD3T', 'BSEG.ZBD2T', 'BSEG.ZBD1T')} AS sk1dtCalc,
  ${dueDateForCashDiscount2('BSEG.KOART', 'BSEG.ZFBDT', 'BKPF.BLDAT', 'BSEG.SHKZG', 'BSEG.REBZG', 'BSEG.ZBD3T', 'BSEG.ZBD2T', 'BSEG.ZBD1T')} AS sk2dtCalc,
  IF(
    BKPF.BUDAT < CURRENT_DATE() AND ${netDueDate('BSEG.KOART', 'BSEG.ZFBDT', 'BKPF.BLDAT', 'BSEG.SHKZG', 'BSEG.REBZG', 'BSEG.ZBD3T', 'BSEG.ZBD2T', 'BSEG.ZBD1T')} > CURRENT_DATE()
    AND BSEG.AUGDT IS NULL, BSEG.DMBTR, 0
  ) AS OpenAndNotDue,
  IF(
    BKPF.BUDAT < CURRENT_DATE() AND BSEG.AUGDT > ${netDueDate('BSEG.KOART', 'BSEG.ZFBDT', 'BKPF.BLDAT', 'BSEG.SHKZG', 'BSEG.REBZG', 'BSEG.ZBD3T', 'BSEG.ZBD2T', 'BSEG.ZBD1T')}
  ...
  IF(
    BKPF.BUDAT < CURRENT_DATE() AND (DATE_DIFF(CURRENT_DATE(), ${netDueDate('BSEG.KOART', 'BSEG.ZFBDT', 'BKPF.BLDAT', 'BSEG.SHKZG', 'BSEG.REBZG', 'BSEG.ZBD3T', 'BSEG.ZBD2T', 'BSEG.ZBD1T')}, DAY) > 90)
    AND BSEG.AUGDT IS NULL, BSEG.DMBTR, 0
  ...
  DATE_DIFF(CURRENT_DATE(), ${netDueDate('BSEG.KOART', 'BSEG.ZFBDT', 'BKPF.BLDAT', 'BSEG.SHKZG', 'BSEG.REBZG', 'BSEG.ZBD3T', 'BSEG.ZBD2T', 'BSEG.ZBD1T')}, DAY)
    AS DaysInArrear,
...
FROM BSEG AS BSEG
INNER JOIN ${ctx.ref(sapDatasetId, 'bkpf')} AS BKPF
...
LEFT JOIN ${ctx.ref(targetDatasetId, 'calendar_date_dim')} AS CalendarDateDimension_H_BUDAT
  ON BKPF.BUDAT = CalendarDateDimension_H_BUDAT.Date
LEFT JOIN ${ctx.ref(targetDatasetId, 'calendar_date_dim')} AS CalendarDateDimension_BLDAT
...
module.exports = {
  isLeapYear,
  netDueDate,
  dueDateForCashDiscount1,
  dueDateForCashDiscount2,
