...
publish("invoice_documents_flow", { ...publishConfig, name: "InvoiceDocuments_Flow" }).query(
  (ctx) => `
  ...
  RSEG AS (
    SELECT
      * EXCEPT (wrbtr, bnkan),
      ...
      END AS bnkan
    FROM
      ${ctx.ref(moduleConfig.sources.sapModule.datasetId, 'rseg')}
  ),
  ...
  RBCO AS (
    SELECT
      * EXCEPT (wrbtr, bnkan_fw),
      ...
      END AS bnkan_fw
    FROM
      ${ctx.ref(moduleConfig.sources.sapModule.datasetId, 'rbco')}
  ),
...
SELECT
  ...
  CAST(NULL AS STRING) AS BudgetPeriod_BUDGET_PD,
  CAST(NULL AS STRING) AS FundedProgram_MEASURE,
  CAST(NULL AS STRING) AS UnitedStatesFederalGovernmentFields_FMFGUS_KEY,
  CAST(NULL AS STRING) AS Accounttype_KOART,
  CAST(NULL AS STRING) AS FinalAccountAssignmentIndicator_AA_FINAL_IND,
  CAST(NULL AS NUMERIC) AS FinalAccountAssignmentQuantity_AA_FINAL_QTY,
  CAST(NULL AS FLOAT64) AS FinalAccountAssignmentQuantityFloat_AA_FINAL_QTY_F,
  CAST(NULL AS NUMERIC) AS ParkedInvoiceQuantity_PARKED_QTY,
  CAST(NULL AS FLOAT64) AS ParkedInvoiceQuantityFloat_PARKED_QTY_F,
  ...
  CAST(NULL AS STRING) AS BlockingReasonItemAmount_SPGRS,
...
FROM RSEG_RBCO
INNER JOIN ${ctx.ref(moduleConfig.sources.sapModule.datasetId, 'rbkp')} AS RBKP
...
LEFT JOIN currency_decimal AS TCURX_WAER
