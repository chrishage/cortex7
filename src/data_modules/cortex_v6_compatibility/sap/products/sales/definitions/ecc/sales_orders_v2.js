...
const publishConfig = publish_config.getPublishConfig(
  ...
  tableConfig,
  moduleConfig,
  ["Client_MANDT", "SalesDocument_VBELN", "Item_POSNR"]
);

publish("sales_orders_v2", { ...publishConfig, name: "SalesOrders_V2" }).query(
  (ctx) => `
WITH TCURX AS (
  ...
  FROM
    ${ctx.ref(moduleConfig.sources.sapModule.datasetId, 'tcurx')}
),
...
AGGVBEP AS (
  ...
  FROM ${ctx.ref(moduleConfig.sources.sapModule.datasetId, 'vbep')}
...
AGGVBPAITEM AS (
  ...
  INNER JOIN ${ctx.ref(moduleConfig.sources.sapModule.datasetId, 'kna1')} AS KNA1
    ON
...
SELECT
  ...
  TIMESTAMP(DATETIME(DATE '1970-01-01', VBAP.HANDOVERTIME)) AS HandOverTimeAtTheHandOverLocation_HANDOVERTIME,
  ...
  EXTRACT(QUARTER FROM VBAK.ERDAT) AS QuarterOfSalesOrderCreationDate_ERDAT,

  EXTRACT(YEAR FROM VBAK.VDATU) AS YearOfRequestedDeliveryDate_VDATU,
  EXTRACT(MONTH FROM VBAK.VDATU) AS MonthOfRequestedDeliveryDate_VDATU,
  EXTRACT(WEEK FROM VBAK.VDATU) AS WeekOfRequestedDeliveryDate_VDATU,
  EXTRACT(DAY FROM VBAK.VDATU) AS DayOfRequestedDeliveryDate_VDATU,
  EXTRACT(QUARTER FROM VBAK.VDATU) AS QuarterOfRequestedDeliveryDate_VDATU,
