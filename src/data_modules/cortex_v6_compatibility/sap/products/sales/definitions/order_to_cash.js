...
publish("order_to_cash", { ...publishConfig, name: "OrderToCash" }).query(
  (ctx) => `
(SELECT
  ...
  IF(
    ...
    TIMESTAMP_DIFF(
      ...
      CAST(CONCAT(SalesOrders.CreationDate_ERDAT, ' ', SalesOrders.CreationTime_ERZET) AS TIMESTAMP), DAY
    ),
    NULL
...
FROM ${ctx.ref('SalesOrders')} AS SalesOrders
LEFT JOIN ${ctx.ref('Deliveries')} AS Deliveries
  ON
...
LEFT JOIN
  ${ctx.ref(moduleConfig.sources.sapMasterData.datasetId, 'CustomersMD')} AS CustomersMD
  ON
    SalesOrders.SoldtoParty_KUNNR = CustomersMD.CustomerNumber_KUNNR
    AND SalesOrders.Client_MANDT = CustomersMD.Client_MANDT
LEFT JOIN
  ${ctx.ref(moduleConfig.sources.sapMasterData.datasetId, 'MaterialsMD')} AS MaterialsMD
  ON
    SalesOrders.MaterialNumber_MATNR = MaterialsMD.MaterialNumber_MATNR
    AND SalesOrders.Client_MANDT = MaterialsMD.Client_MANDT
LEFT JOIN
  ${ctx.ref(moduleConfig.sources.sapMasterData.datasetId, 'SalesOrganizationsMD')} AS SalesOrganizationsMD
  ON
...
LEFT JOIN
  ${ctx.ref(moduleConfig.sources.sapMasterData.datasetId, 'DistributionChannelsMD')} AS DistributionChannelMD
  ON
...
LEFT JOIN
  ${ctx.ref(moduleConfig.sources.sapMasterData.datasetId, 'CountriesMD')} AS CountriesMD
  ON
...
-- if not using the view
LEFT JOIN
  ${ctx.ref('OneTouchOrder')} AS OneTouchOrder
  ON
...
LEFT JOIN
  ${ctx.ref(moduleConfig.sources.sapMasterData.datasetId, 'DivisionsMD')} AS DivisionsMD
...
-- LEFT JOIN ${ctx.ref(moduleConfig.sources.sapModule.datasetId, 'dd07t')} AS DD07T
...
LEFT JOIN ${ctx.ref(moduleConfig.sources.sapMasterData.datasetId, 'DeliveryBlockingReasonsMD')} AS TVLST
  ON
...
LEFT JOIN ${ctx.ref(moduleConfig.sources.sapMasterData.datasetId, 'BillingBlockingReasonsMD')} AS TVFST
  ON
...
WHERE MaterialsMD.Language_SPRAS IN UNNEST(${JSON.stringify(languages)})
  AND DivisionsMD.LanguageKey_SPRAS IN UNNEST(${JSON.stringify(languages)}))
`
);
