...
publish("fiscal_date_dim", { ...publishConfig, name: "fiscal_date_dim" }).query(
  (ctx) => `
...
(
  ...
  FROM
    ${ctx.ref(moduleConfig.sources.sapModule.datasetId, 't009')},
    UNNEST(
      GENERATE_DATE_ARRAY(
  ...
  WITH
    ...
    GetAggFields AS (
      SELECT
        ...
        WeekStartDate,
        WeekEndDate,
        LPAD(CAST(poper AS STRING), 3, '0') AS FiscalPeriod,
        ...
        FORMAT_DATE('%A', dt) AS DayNameLong,
        FORMAT_DATE('%a', dt) AS DayNameShort,
        CONCAT(FiscalYear, LPAD(CAST(poper AS STRING), 3, '0')) AS FiscalYearPeriod
  ...
  WITH
    ...
    GetEndDateForFirstScenario AS (
      SELECT
        ...
        CASE
          WHEN NOT ${finance.isLeapYear('cal_year')}
            AND SAFE_CAST(t009b.bumon AS INT64) = 2 AND SAFE_CAST(t009b.butag AS INT64) = 29
      ...
      WHERE t009.mandt = '${mandt}'
        ...
        AND SAFE_CAST(t009b.bdatj AS STRING) = '0000'
    ),
      ...
      SELECT
        ...
        DATE_SUB(
          DATE_TRUNC(
            CAST(
              CASE
                WHEN SAFE_CAST(MinimumReljr AS INT64) > 0
                  THEN CONCAT(FiscalYear - SAFE_CAST(MinimumReljr AS INT64), '-', MinimumPeriod)
                ELSE CONCAT(FiscalYear, '-', MinimumPeriod)
              END AS DATE),
            MONTH),
        ...
        CAST(
          CASE
            WHEN SAFE_CAST(MinimumReljr AS INT64) < 0
              THEN CONCAT(FiscalYear - SAFE_CAST(MinimumReljr AS INT64), '-', MaximumPeriod)
            ELSE CONCAT(FiscalYear, '-', MaximumPeriod)
