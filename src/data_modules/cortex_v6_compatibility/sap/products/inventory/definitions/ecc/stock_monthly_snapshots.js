...
WITH
  LastSnapshots AS (
    ${ctx.when(ctx.incremental(),
      `SELECT
        ...
        amount_monthly_cumulative AS start_amount
      FROM
        ${ctx.self()}
      WHERE
  ...
  NewMovements AS (
    ...
    WHERE
      1 = 1
      ${ctx.when(ctx.incremental(), `AND src.budat_mkpf > (SELECT MAX(month_end_date) FROM ${ctx.self()})`)}
    GROUP BY
      src.mandt,
      src.werks,
      src.matnr,
