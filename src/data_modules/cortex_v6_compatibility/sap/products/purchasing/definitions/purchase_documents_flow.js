...
publish("purchase_documents_flow", { ...publishConfig, name: "PurchaseDocuments_Flow" }).query(
  (ctx) => `
WITH
  currency_decimal AS (
  ...
  EKKN AS (
    ...
    FROM ${ctx.ref(moduleConfig.sources.sapModule.datasetId, 'ekkn')} AS EKKN
    INNER JOIN ${ctx.ref(moduleConfig.sources.sapModule.datasetId, 'ekpo')} AS EKPO
      USING (mandt, ebelp, ebeln)
  ),
  ...
  GoodsReceiptStaging AS (
    ...
    FROM ${ctx.ref(moduleConfig.sources.sapModule.datasetId, 'ekbe')}
    WHERE VGABE = '1'
    GROUP BY 1, 2, 3, 4
  ),
...
SELECT
  ...
  CASE
    WHEN CONV.TCURR = '${targetCurrencies[0]}'
...
FROM ${ctx.ref(moduleConfig.sources.sapModule.datasetId, 'ekko')} AS EKKO
LEFT JOIN ${ctx.ref(moduleConfig.sources.sapModule.datasetId, 'ekpo')} AS EKPO
  ON
...
LEFT JOIN CONV
  ON EKKO.MANDT = CONV.MANDT
    ...
    AND CONV.TCURR = '${targetCurrencies[0]}'
`
);
