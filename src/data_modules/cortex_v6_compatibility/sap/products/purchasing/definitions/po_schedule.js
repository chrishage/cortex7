...
publish("po_schedule", { ...publishConfig, name: "POSchedule" }).query(
  (ctx) => `
...
SELECT
  ...
  TIMESTAMP(DATETIME(DATE '1970-01-01', eket.MBUHR)) AS MaterialStagingTime_MBUHR,
  eket.LDDAT AS LoadingDate_LDDAT,
  TIMESTAMP(DATETIME(DATE '1970-01-01', eket.LDUHR)) AS LoadingTime__localTimeRelatingToAShippingPoint___LDUHR,
  eket.TDDAT AS TransportationPlanningDate_TDDAT,
  TIMESTAMP(DATETIME(DATE '1970-01-01', eket.TDUHR)) AS TranspPlanningTime__local_TDUHR,
  eket.WADAT AS GoodsIssueDate_WADAT,
  TIMESTAMP(DATETIME(DATE '1970-01-01', eket.WAUHR)) AS TimeOfGoodsIssue__local_RelatingToAPlant___WAUHR,
  ...
  TIMESTAMP(DATETIME(DATE '1970-01-01', eket.HANDOVERTIME)) AS HandoverTimeAtTheHandoverLocation_HANDOVERTIME,
  ...
  CalendarDateDimension_EINDT.cal_year AS YearOfItemDeliveryDate_EINDT,
  CalendarDateDimension_EINDT.cal_month AS MonthOfItemDeliveryDate_EINDT,
  CalendarDateDimension_EINDT.cal_week AS WeekOfItemDeliveryDate_EINDT,
  CalendarDateDimension_EINDT.cal_quarter AS QuarterOfItemDeliveryDate_EINDT,
  CalendarDateDimension_BEDAT.cal_year AS YearOfOrderDateOfScheduleLine_BEDAT,
  CalendarDateDimension_BEDAT.cal_month AS MonthOfOrderDateOfScheduleLine_BEDAT,
  CalendarDateDimension_BEDAT.cal_week AS WeekOfOrderDateOfScheduleLine_BEDAT,
  CalendarDateDimension_BEDAT.cal_quarter AS QuarterOfOrderDateOfScheduleLine_BEDAT
FROM
  ${ctx.ref(moduleConfig.sources.sapModule.datasetId, 'eket')} AS eket
LEFT JOIN calendar_date_dim AS CalendarDateDimension_EINDT
  ON CalendarDateDimension_EINDT.Date = eket.EINDT
LEFT JOIN calendar_date_dim AS CalendarDateDimension_BEDAT
