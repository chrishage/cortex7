...
publish("accounting_documents", { ...publishConfig, name: "AccountingDocuments" }).query(
  (ctx) => `
...
BSEG AS (
    ...
    SELECT
      * EXCEPT (
      ...
      END AS PENFC
    FROM
      ${ctx.ref(moduleConfig.sources.sapModule.datasetId, 'bseg')}
  )
...
SELECT
  ...
  TIMESTAMP(DATETIME(DATE '1970-01-01', BKPF.CPUTM)) AS TimeOfEntry_CPUTM,
  ...
  TIMESTAMP(DATETIME(DATE '1970-01-01', BKPF.PSOTM)) AS LastChangedAt_PSOTM,
  ...
  CAST(COALESCE(BSEG.DMBTR * TCURXHWAER.CURRFIX, BSEG.DMBTR) AS NUMERIC) AS AmountInLocalCurrency_DMBTR,
  CAST(COALESCE(BSEG.WRBTR * TCURXWAERS.CURRFIX, BSEG.WRBTR) AS NUMERIC) AS AmountInDocumentCurrency_WRBTR,
  CAST(COALESCE(BSEG.KZBTR * TCURXHWAER.CURRFIX, BSEG.KZBTR) AS NUMERIC) AS OriginalReductionAmountInLocalCurrency_KZBTR,
  CAST(COALESCE(BSEG.PSWBT * TCURXPSWSL.CURRFIX, BSEG.PSWBT) AS NUMERIC) AS AmountForUpdatingInGeneralLedger_PSWBT,
  CAST(COALESCE(BSEG.TXBHW * TCURXHWAER.CURRFIX, BSEG.TXBHW) AS NUMERIC) AS OriginalTaxBaseAmountInLocalCurrency_TXBHW,
  CAST(COALESCE(BSEG.TXBFW * TCURXWAERS.CURRFIX, BSEG.TXBFW) AS NUMERIC) AS OriginalTaxBaseAmountInDocumentCurrency_TXBFW,
  CAST(COALESCE(BSEG.MWSTS * TCURXHWAER.CURRFIX, BSEG.MWSTS) AS NUMERIC) AS TaxAmountInLocalCurrency_MWSTS,
  CAST(COALESCE(BSEG.WMWST * TCURXWAERS.CURRFIX, BSEG.WMWST) AS NUMERIC) AS TaxAmountInDocumentCurrency_WMWST,
  CAST(COALESCE(BSEG.HWBAS * TCURXHWAER.CURRFIX, BSEG.HWBAS) AS NUMERIC) AS TaxBaseAmountInLocalCurrency_HWBAS,
  CAST(COALESCE(BSEG.FWBAS * TCURXWAERS.CURRFIX, BSEG.FWBAS) AS NUMERIC) AS TaxBaseAmountInDocumentCurrency_FWBAS,
  CAST(COALESCE(BSEG.HWZUZ * TCURXHWAER.CURRFIX, BSEG.HWZUZ) AS NUMERIC) AS ProvisionAmountInLocalCurrency_HWZUZ,
  CAST(COALESCE(BSEG.QSSHB * TCURXWAERS.CURRFIX, BSEG.QSSHB) AS NUMERIC) AS WithholdingTaxBaseAmount_QSSHB,
  CAST(COALESCE(BSEG.GBETR * TCURXHWAER.CURRFIX, BSEG.GBETR) AS NUMERIC) AS HedgedAmountInForeignCurrency_GBETR,
  CAST(COALESCE(BSEG.SKFBT * TCURXWAERS.CURRFIX, BSEG.SKFBT) AS NUMERIC) AS AmountEligibleForCashDiscountInDocumentCurrency_SKFBT,
  CAST(COALESCE(BSEG.SKNTO * TCURXHWAER.CURRFIX, BSEG.SKNTO) AS NUMERIC) AS CashDiscountAmountInLocalCurrency_SKNTO,
  CAST(COALESCE(BSEG.WSKTO * TCURXWAERS.CURRFIX, BSEG.WSKTO) AS NUMERIC) AS CashDiscountAmountInDocumentCurrency_WSKTO,
  CAST(COALESCE(BSEG.NEBTR * TCURXWAERS.CURRFIX, BSEG.NEBTR) AS NUMERIC) AS NetPaymentAmount_NEBTR,
  CAST(COALESCE(BSEG.DMBT1 * TCURXHWAER.CURRFIX, BSEG.DMBT1) AS NUMERIC) AS AmountInLocalCurrencyForTaxDistribution_DMBT1,
  CAST(COALESCE(BSEG.WRBT1 * TCURXHWAER.CURRFIX, BSEG.WRBT1) AS NUMERIC) AS AmountInForeignCurrencyForTaxBreakdown_WRBT1,
  CAST(COALESCE(BSEG.DMBT2 * TCURXHWAE2.CURRFIX, BSEG.DMBT2) AS NUMERIC) AS AmountInLocalCurrencyForTaxDistribution_DMBT2,
  CAST(COALESCE(BSEG.WRBT2 * TCURXHWAE2.CURRFIX, BSEG.WRBT2) AS NUMERIC) AS AmountInForeignCurrencyForTaxBreakdown_WRBT2,
  CAST(COALESCE(BSEG.DMBT3 * TCURXHWAE3.CURRFIX, BSEG.DMBT3) AS NUMERIC) AS AmountInLocalCurrencyForTaxDistribution_DMBT3,
  CAST(COALESCE(BSEG.WRBT3 * TCURXHWAE3.CURRFIX, BSEG.WRBT3) AS NUMERIC) AS AmountInForeignCurrencyForTaxBreakdown_WRBT3,
  CAST(COALESCE(BSEG.KLIBT * TCURXHWAER.CURRFIX, BSEG.KLIBT) AS NUMERIC) AS CreditControlAmount_KLIBT,
  CAST(COALESCE(BSEG.QBSHB * TCURXWAERS.CURRFIX, BSEG.QBSHB) AS NUMERIC) AS WithholdingTaxAmount__inDocumentCurrency___QBSHB,
  CAST(COALESCE(BSEG.QSFBT * TCURXWAERS.CURRFIX, BSEG.QSFBT) AS NUMERIC) AS WithholdingTaxExemptAmount_QSFBT,
  CAST(COALESCE(BSEG.BONFB * TCURXHWAER.CURRFIX, BSEG.BONFB) AS NUMERIC) AS AmountQualifyingForBonusInLocalCurrency_BONFB,
  CAST(COALESCE(BSEG.DMBE2 * TCURXHWAE2.CURRFIX, BSEG.DMBE2) AS NUMERIC) AS AmountInSecondLocalCurrency_DMBE2,
  ...
  CAST(COALESCE(BSEG.DMBE3 * TCURXHWAE3.CURRFIX, BSEG.DMBE3) AS NUMERIC) AS AmountInThirdLocalCurrency_DMBE3,
  CAST(COALESCE(BSEG.DMB21 * TCURXHWAE2.CURRFIX, BSEG.DMB21) AS NUMERIC) AS AmountInSecondLocalCurrencyForTaxBreakdown_DMB21,
  CAST(COALESCE(BSEG.DMB22 * TCURXHWAE2.CURRFIX, BSEG.DMB22) AS NUMERIC) AS AmountInSecondLocalCurrencyForTaxBreakdown_DMB22,
  CAST(COALESCE(BSEG.DMB23 * TCURXHWAE2.CURRFIX, BSEG.DMB23) AS NUMERIC) AS AmountInSecondLocalCurrencyForTaxBreakdown_DMB23,
  CAST(COALESCE(BSEG.DMB31 * TCURXHWAE3.CURRFIX, BSEG.DMB31) AS NUMERIC) AS AmountInThirdLocalCurrencyForTaxBreakdown_DMB31,
  CAST(COALESCE(BSEG.DMB32 * TCURXHWAE3.CURRFIX, BSEG.DMB32) AS NUMERIC) AS AmountInThirdLocalCurrencyForTaxBreakdown_DMB32,
  CAST(COALESCE(BSEG.DMB33 * TCURXHWAE3.CURRFIX, BSEG.DMB33) AS NUMERIC) AS AmountInThirdLocalCurrencyForTaxBreakdown_DMB33,
  CAST(COALESCE(BSEG.MWST2 * TCURXHWAE2.CURRFIX, BSEG.MWST2) AS NUMERIC) AS TaxAmountInSecondLocalCurrency_MWST2,
  CAST(COALESCE(BSEG.MWST3 * TCURXHWAE3.CURRFIX, BSEG.MWST3) AS NUMERIC) AS TaxAmountInThirdLocalCurrency_MWST3,
  CAST(COALESCE(BSEG.NAVH2 * TCURXHWAE2.CURRFIX, BSEG.NAVH2) AS NUMERIC) AS NonDeductibleInputTaxInSecondLocalCurrency_NAVH2,
  CAST(COALESCE(BSEG.NAVH3 * TCURXHWAE3.CURRFIX, BSEG.NAVH3) AS NUMERIC) AS NonDeductibleInputTaxInThirdLocalCurrency_NAVH3,
  CAST(COALESCE(BSEG.SKNT2 * TCURXHWAE2.CURRFIX, BSEG.SKNT2) AS NUMERIC) AS CashDiscountAmountInSecondLocalCurrency_SKNT2,
  CAST(COALESCE(BSEG.SKNT3 * TCURXHWAE3.CURRFIX, BSEG.SKNT3) AS NUMERIC) AS CashDiscountAmountInThirdLocalCurrency_SKNT3,
  CAST(COALESCE(BSEG.STTAX * TCURXWAERS.CURRFIX, BSEG.STTAX) AS NUMERIC) AS TaxAmountAsStatisticalInformationInDocumentCurrency_STTAX,
  CAST(COALESCE(BSEG.PENLC1 * TCURXHWAER.CURRFIX, BSEG.PENLC1) AS NUMERIC) AS PenaltyChargeAmountInFirstLocalCurrency_PENLC1,
  CAST(COALESCE(BSEG.PENLC2 * TCURXHWAE2.CURRFIX, BSEG.PENLC2) AS NUMERIC) AS PenaltyChargeAmountInSecondLocalCurrency_PENLC2,
  CAST(COALESCE(BSEG.PENLC3 * TCURXHWAE3.CURRFIX, BSEG.PENLC3) AS NUMERIC) AS PenaltyChargeAmountInThirdLocalCurrency_PENLC3,
  CAST(COALESCE(BSEG.PENFC * TCURXWAERS.CURRFIX, BSEG.PENFC) AS NUMERIC) AS PenaltyChargeAmountInDocumentCurrency_PENFC,
  ...
  CAST(IF(BSEG.AUGDT IS NULL, COALESCE(BSEG.DMBTR * TCURXHWAER.CURRFIX, BSEG.DMBTR), 0) AS NUMERIC) AS AmountInLocalCurrencyClearingDate_DMBTR,
  ${finance.netDueDate('BSEG.KOART', 'BSEG.ZFBDT', 'BKPF.BLDAT', 'BSEG.SHKZG', 'BSEG.REBZG', 'BSEG.ZBD3T', 'BSEG.ZBD2T', 'BSEG.ZBD1T')} AS NetDueDateCalc,
  ${finance.dueDateForCashDiscount1('BSEG.KOART', 'BSEG.ZFBDT', 'BKPF.BLDAT', 'BSEG.SHKZG', 'BSEG.REBZG', 'BSEG.ZBD3T', 'BSEG.ZBD2T', 'BSEG.ZBD1T')} AS Sk1DtCalc,
  ${finance.dueDateForCashDiscount2('BSEG.KOART', 'BSEG.ZFBDT', 'BKPF.BLDAT', 'BSEG.SHKZG', 'BSEG.REBZG', 'BSEG.ZBD3T', 'BSEG.ZBD2T', 'BSEG.ZBD1T')} AS Sk2DtCalc,
  IF(
    BKPF.BUDAT < CURRENT_DATE() AND ${finance.netDueDate('BSEG.KOART', 'BSEG.ZFBDT', 'BKPF.BLDAT', 'BSEG.SHKZG', 'BSEG.REBZG', 'BSEG.ZBD3T', 'BSEG.ZBD2T', 'BSEG.ZBD1T')} > CURRENT_DATE()
    AND BSEG.AUGDT IS NULL, BSEG.DMBTR, 0
  ) AS OpenAndNotDue,
  IF(
    BKPF.BUDAT < CURRENT_DATE() AND BSEG.AUGDT > ${finance.netDueDate('BSEG.KOART', 'BSEG.ZFBDT', 'BKPF.BLDAT', 'BSEG.SHKZG', 'BSEG.REBZG', 'BSEG.ZBD3T', 'BSEG.ZBD2T', 'BSEG.ZBD1T')}
  ...
  IF(
    BKPF.BUDAT < CURRENT_DATE() AND (DATE_DIFF(CURRENT_DATE(), ${finance.netDueDate('BSEG.KOART', 'BSEG.ZFBDT', 'BKPF.BLDAT', 'BSEG.SHKZG', 'BSEG.REBZG', 'BSEG.ZBD3T', 'BSEG.ZBD2T', 'BSEG.ZBD1T')}, DAY) > 90)
    AND BSEG.AUGDT IS NULL, BSEG.DMBTR, 0
  ...
  DATE_DIFF(CURRENT_DATE(), ${finance.netDueDate('BSEG.KOART', 'BSEG.ZFBDT', 'BKPF.BLDAT', 'BSEG.SHKZG', 'BSEG.REBZG', 'BSEG.ZBD3T', 'BSEG.ZBD2T', 'BSEG.ZBD1T')}, DAY)
    AS DaysInArrear,
...
LEFT JOIN currency_decimal AS TCURXWAERS
  ON BKPF.WAERS = TCURXWAERS.CURRKEY
LEFT JOIN currency_decimal AS TCURXHWAER
  ON BKPF.HWAER = TCURXHWAER.CURRKEY
LEFT JOIN currency_decimal AS TCURXHWAE2
  ON BKPF.HWAE2 = TCURXHWAE2.CURRKEY
LEFT JOIN currency_decimal AS TCURXHWAE3
  ON BKPF.HWAE3 = TCURXHWAE3.CURRKEY
LEFT JOIN currency_decimal AS TCURXPSWSL
  ON BSEG.PSWSL = TCURXPSWSL.CURRKEY
LEFT JOIN ${ctx.ref(moduleConfig.sources.sapMasterData.datasetId, 'calendar_date_dim')} AS CalendarDateDimension_H_BUDAT
  ON BKPF.BUDAT = CalendarDateDimension_H_BUDAT.Date
LEFT JOIN ${ctx.ref(moduleConfig.sources.sapMasterData.datasetId, 'calendar_date_dim')} AS CalendarDateDimension_BLDAT
